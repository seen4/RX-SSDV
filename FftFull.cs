...
{
    /*
    Origin: NWaves.Transform.Fft
    由于Fft的Direct无法计算负数频率部分频谱，所以做了这个类来写一个支持负数频率的拓展
    */
    /// <summary>
    /// Just like Fft of NWaves, but this support negative frequency calculation.
    /// </summary>
    public class FftFull : IComplexTransform
    {
        /// <summary>
        /// Gets FFT size.
        /// </summary>
        ...
        /// <summary>
        /// Precomputed cosines(Negative).
        /// </summary>
        private readonly float[] _cosTblN;

        /// <summary>
        /// Precomputed sines(Negaive).
        /// </summary>
        private readonly float[] _sinTblN;
        ...
        public FftFull(int fftSize = 512)
        {
            ...
            _cosTblN = new float[tblSize];
            _sinTblN = new float[tblSize];
            ...
            for (int i = 1, pos = 0; i < _fftSize; i *= 2, pos++)
            {
                ...
                _cosTblN[pos] = (float)Math.Cos(2 * Math.PI * i / _fftSize * -1);
                _sinTblN[pos] = (float)Math.Sin(2 * Math.PI * i / _fftSize * -1);
            }
        }
        ...
        /// <summary>
        /// Does Fast Fourier Transform in-place(Negative frequency).
        /// </summary>
        /// <param name="re">Array of real parts</param>
        /// <param name="im">Array of imaginary parts</param>
        public void DirectNegative(float[] re, float[] im)
        {
            var L = _fftSize;
            var M = _fftSize >> 1;
            var S = _fftSize - 1;
